		...
		if ( result && result.query && result.query.pages ) {
			$.each( result.query.pages, function ( index, pageInfo ) {
				var hiddenStatus = !!( pageInfo.pageprops && pageInfo.pageprops.hiddencat !== undefined ),
					redirectInfo = $.grep( result.query.redirects, function ( element ) {
						return element.to.indexOf( pageInfo.title ) !== -1;
					} );
				if ( redirectInfo.length ) {
					categoryWidget.categoryHiddenStatus[redirectInfo[0].from] = hiddenStatus;
				} else {
